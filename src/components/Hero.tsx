...
export default function Hero() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Animate over the first viewport height
  ...
  const scale = 1 - scrollProgress * 0.2; // Scale from 1 to 0.8
  const translateY = scrollProgress * 100; // Move down by 100px
  const opacity = 1 - scrollProgress * 0.7; // Fade out
  const blur = scrollProgress * 4; // Add blur effect

  return (
    <>
      {/* Desktop Version */}
      <div 
        className="hidden lg:block fixed top-0 left-0 w-full h-screen z-0 transition-transform duration-75 ease-out"
        style={{
        ...
        <div className="relative bg-white overflow-hidden h-screen">
          ...
          <div className="relative z-10 h-full flex items-center">
            <div className="px-6 lg:px-12 xl:px-16">
              <div className="max-w-xl">
                <h1 
                  className="text-4xl lg:text-6xl font-light text-white mb-8 leading-tight tracking-tight animate-fade-in-up"
                ...
                <p 
                  className="text-lg lg:text-xl text-gray-200 mb-10 font-light animate-fade-in-up delay-300"
                  style={{ opacity: 1 - scrollProgress * 1.5 }}
                >
                  Learn Java, Web Development, Digital Marketing & more – Now in Kadapa!
                </p>
      ...
      {/* Mobile Version - Buttons at Bottom, Larger Text */}
      <div 
        className="block lg:hidden fixed top-0 left-0 w-full min-h-screen z-0 transition-transform duration-75 ease-out"
        style={{
          transform: `scale(${scale}) translateY(${translateY}px)`,
          opacity: opacity,
          filter: `blur(${blur}px)`,
      ...
      >
        <div className="relative bg-white overflow-hidden min-h-screen">
          ...
          <div className="relative z-10 min-h-screen flex flex-col px-6">
            ...
            {/* Buttons Container - Fixed at Bottom */}
            <div 
              className="pb-8 animate-fade-in-up delay-500"
              style={{ opacity: 1 - scrollProgress * 1.5 }}
            >
              <div className="flex flex-col gap-4">
                <a 
